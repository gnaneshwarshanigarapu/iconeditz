...
export default function ServicesEditor({ content, update, onSave }) {
    return (
        <div className="space-y-4">
            <p className="text-sm text-gray-400">Manage your services below. Add, remove, and reorder items.</p>
            {(content.items || []).map((service, index) => (
                <div key={index} className="rounded-lg border border-white/10 p-3">
                    <FormInput
                        label={`Service ${index + 1} Title`}
                        value={service.title}
                        onChange={e => update(draft => { draft.items[index].title = e.target.value; })}
                    />
                    <FormInput
                        label="Description"
                        type="textarea"
                        value={service.description}
                        onChange={e => update(draft => { draft.items[index].description = e.target.value; })}
                    />
                </div>
            ))}
