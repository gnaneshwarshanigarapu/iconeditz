...
export default function TestimonialsEditor({ content, update, onSave }) {
    return (
            ...
            {(content.items || []).map((testimonial, index) => (
                <div key={index} className="rounded-lg border border-white/10 p-3">
                    <FormInput
                        ...
                        onChange={e => update(draft => { draft.items[index].name = e.target.value; })}
                    />
                    <FormInput
                        label="Company"
                        value={testimonial.company}
                        onChange={e => update(draft => { draft.items[index].company = e.target.value; })}
                    />
                    <FormInput
                        label="Review"
                        type="textarea"
                        value={testimonial.review}
                        onChange={e => update(draft => { draft.items[index].review = e.target.value; })}
                    />
                </div>
            ))}
