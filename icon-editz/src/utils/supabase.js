...
export const signIn = async (email, password) => { const result = await request('/api/auth/login', { method: 'POST', body: { email, password }, token: null }); localStorage.setItem('token', result.token); return { data: { user: result.user, session: { access_token: result.token } }, error: null } }
export const signOut = async () => { localStorage.removeItem('token'); return { error: null } }
...
export const upsertProduct = async product => ({ data: await request(product.id ? `/api/products/${product.id}` : '/api/products', { method: product.id ? 'PUT' : 'POST', body: product }), error: null })
export const deleteProduct = async id => ({ data: await request(`/api/products/${id}`, { method: 'DELETE' }), error: null })
export const toggleProductPublish = async (id, published) => ({ data: await request(`/api/products/${id}`, { method: 'PUT', body: { published } }), error: null })
