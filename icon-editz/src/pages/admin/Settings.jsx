...
export default function Settings() {
  ...
  useEffect(() => { request('/api/settings').then((response) => reset({ analytics: response.data || {} })).catch((error) => setNotice(error.message)).finally(() => setLoading(false)) }, [reset])
  const submit = async ({ analytics }) => { setNotice(''); try { await request('/api/settings', { method: 'PUT', body: { settings: analytics } }); setNotice('Settings saved successfully.') } catch (error) { setNotice(error.message) } }
